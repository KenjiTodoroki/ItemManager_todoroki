...
public class UserDAO {
  ...
  public UserBean checkLogin(String id, String password) throws ClassNotFoundException, SQLException {
    ...
    try (Connection con = ConnectionManager.getConnection();
      ...
      // プレースホルダに値をセット 1からスタート(配列と違い0はなし)
      pstmt.setString(1, id);
      pstmt.setString(2, password);
			
      /*
       *  SQL文の実行
       *  executeQuery()はSQL文がSELECTの場合のみ[参照系]
       *  insert, update, deleteの場合はexecuteUpdate()になる[更新系]
       */
      ...
      if (res.next()) {
        user = new UserBean();
        user.setUserId(res.getString("user_id"));	// "user_id" = カラム名
        user.setPassword(res.getString("password"));	// "password" = カラム名
      }
    }
    return user;
  }
